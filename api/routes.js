...
app.get('/query/:param', async (req, res) => {
    db.single(q.selectFromDB, [req.params.param])
    .then(data => {
    if(data.rows.length) {
      res.send(data.rows)
    } else {
  ...
  app.get('/shelf/:param', async (req, result) => {
    var qarr = req.params.param.split(',')
     ...
     var qres = await db.single(q.updateFavoriteQuery, request)
     result.send(qres.rows)
  })
